...
(int[,], bool) GetMatrixProduct(int[,] m1, int[,] m2)
{
...
var matrixA = FillTable(rows: 2, cols: 3, minRange: -10, maxRange: 10);
var matrixB = FillTable(rows: 3, cols: 2, minRange: -10, maxRange: 10);
 
Console.WriteLine("\nМатрица 1.");
PrintMatrixMyWay(matrixA);
Console.WriteLine("\nМатрица 2.");
PrintMatrixMyWay(matrixB);
 
(int[,] product, bool success) = GetMatrixProduct(matrixA, matrixB);
if (success)
{
    Console.WriteLine("Произведение матриц:");
    PrintMatrixMyWay(product);
}
else
{
    Console.WriteLine($"Данные матрицы нельзя умножить, так как число стобцов первой матрицы {matrixA.GetLongLength(1)} не совпадает с числом строк второй {matrixB.GetLongLength(0)}.\n");
}